...
export default function NetworkGraph({ profileData }: NetworkGraphProps) {
  const fgRef = useRef<any>();
  const [graphData, setGraphData] = useState<GraphDataState>({ nodes: [], links: [] });
    ...
    <div className="w-full h-full relative bg-[#020a18] rounded-xl overflow-hidden border border-cyan-900/30">
      ...
      <div className="absolute inset-0 z-0">
        <ForceGraph3D
          ...
          nodeResolution={16}
          linkColor={(link: any) => link.dashed ? '#6b7280' : '#38bdf8'}
          linkWidth={(link: any) => link.strength ? link.strength * 2 : 1}
          linkResolution={8}
          linkDirectionalParticles={2}
          linkDirectionalParticleWidth={1.5}
          linkDirectionalParticleSpeed={(d: any) => d.strength * 0.01}
